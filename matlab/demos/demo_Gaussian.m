...
%% simulate Gaussian traces
% model parameters
prs.C = bsxfun(@times,randn(dy,dx),[1 1.2 1.3]);
prs.b = 0.2*ones(dy,1);
prs.psi = 2*ones(dy,1); % noise variance for Gaussian likelihood (all covariance captured in latent -- for now)
% grids for plotting
Nmax = 500;
dt = max(T)/Nmax;
...
for nn = 1:ntr
    ...
    Y{nn} = prs.C*latents + prs.b + diag(sqrt(prs.psi))*randn(dy,length(t));
...
options.parallel = 0;
options.verbose = 1;
m = InitialiseModel_svGPFA('Gaussian',Y,trLen,kerns,Z,prs,options);
